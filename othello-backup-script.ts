...
const yellowSprite = Sprite.fromImage("./yellowkris.png");
var emptyBox = PIXI.Texture.fromImage("./emptybox.png");
var legalBox = PIXI.Texture.fromImage("./legalbox.png");

// const defaultIcon = "url('bluekris.png'),auto";
// app.renderer.plugins.interaction.cursorStyles.default = defaultIcon;

// app.stage.addChild(defaultIcon);
...
class Tile {
    ...
    setColor(): void { // sets color based on the value of the tile
        if (this.value === 0) {
            ...
            // inconsistency lol
            // the inconsistency is here temporarily since I have two ways  
            // of making this work and I want to see if one is better
        } else if (this.value === 3) {
            this.sprite = Sprite.fromImage("./yellowkris.png");
                ...
                if (oneTurnPrediction.length === 0) { // if the next player has no legal moves
                    ...
                    if (twoTurnPrediction.length === 0) { // if the next two players have no legal moves
                        ...
                        if (threeTurnPrediction.length === 0) { // if the next three players have no legal moves
                            console.log("checkpoint 3" + threeTurnPrediction);
    ...
    place = (): void => {
        ...
        this.setColor();
        // console.log("Place Function was called.");
    }
}
...
let detect = (color: number, tileY: number, tileX: number, ydir: number, xdir: number): boolean => {
    ...
    // console.log(color + " " + tileY + " " + tileX + " " + ydir + " " + xdir);
